...
constexpr bool is_log_enabled = false;
...
float Psu::power_consumed()
{
    ...
    read_2(0x16, 4.0f * 3600, "Wh", "Power consumed");
}
...
float Psu::read_2(uint8_t reg, float scale, std::string unit, std::string name)
{
    float value = read(reg);
    float value_1 = read(reg + 1);
    value = (int32_t)value_1 << 16 | (int32_t)value;
...
float Psu::read(uint8_t reg)
{
    ...
    size_t n = Wire.requestFrom(_address, 2);
